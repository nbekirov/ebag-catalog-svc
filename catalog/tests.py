...
class ProductListTests(APITestCase):
                ...
                "results": [
                    {
                        ...
                        "created_at": products[0]
                        .created_at.isoformat()
                        .replace("+00:00", "Z"),
                        "updated_at": products[0]
                    ...
                    },
                    {
                        ...
                        "currency": "EUR",
                        "category_id": category.id,
                        "created_at": products[1]
                        .created_at.isoformat()
                        .replace("+00:00", "Z"),
                        "updated_at": products[1]
                        .updated_at.isoformat()
                        .replace("+00:00", "Z"),
                    },
                    {
                        ...
                        "currency": "EUR",
                        "category_id": category.id,
                        "created_at": products[2]
                        .created_at.isoformat()
                        .replace("+00:00", "Z"),
                        "updated_at": products[2]
                        .updated_at.isoformat()
                        .replace("+00:00", "Z"),
                    },
                ],
    ...
    def test_paginated(self):
        ...
        products = [
            Product.objects.create(
                title=f"P{i}", sku=f"SKU-{i}", price_cents=99, category=category
            )
            for i in range(5)
        ]
...
class ProductSearchTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.groceries, cls.drinks, cls.juice, cls.fish, cls.fillets = (
            groceries,
            drinks,
            juice,
            fish,
    ...
    def test_matches_title_ignoring_case(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            [product["id"] for product in response.data["results"]], [self.mackerel.id]
        )
    ...
    def test_combines_title_and_sku(self):
        response = self.client.get(
            reverse("product-list"), {"title": "water", "sku": "DRK-WAT-001"}
        )
    ...
    def test_matches_price_within_range(self):
        response = self.client.get(
            reverse("product-list"), {"price_cents_min": 129, "price_cents_max": 349}
        )
    ...
    def test_finds_nothing_when_minimum_exceeds_maximum(self):
        response = self.client.get(
            reverse("product-list"), {"price_cents_min": 349, "price_cents_max": 129}
        )
    ...
    def test_rejects_negative_price(self):
        ...
        self.assertEqual(
            response.data,
            {"price_cents_min": ["Ensure this value is greater than or equal to 0."]},
        )
    ...
    def test_rejects_non_integer_price(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data, {"price_cents_max": ["A valid integer is required."]}
        )
    ...
    def test_matches_category_including_subcategories(self):
        response = self.client.get(
            reverse("product-list"), {"category": self.drinks.id}
        )
    ...
    def test_matches_top_level_category_including_all_levels(self):
        response = self.client.get(
            reverse("product-list"), {"category": self.groceries.id}
        )
    ...
    def test_combines_category_and_title(self):
        response = self.client.get(
            reverse("product-list"), {"category": self.fish.id, "title": "fillet"}
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            [product["id"] for product in response.data["results"]], [self.sea_bass.id]
        )

    def test_keeps_filters_in_pagination_links(self):
        response = self.client.get(
            reverse("product-list"), {"category": self.drinks.id, "limit": 2}
        )
...
class CategoryCreateTests(APITestCase):
    ...
    def test_creates_child(self):
        ...
        response = self.client.post(
            reverse("category-list"), {"name": "Water", "parent_id": drinks.id}
        )
    ...
    def test_creates_same_name_under_different_parent(self):
        ...
        response = self.client.post(
            reverse("category-list"), {"name": "Organic", "parent_id": food.id}
        )
    ...
    def test_rejects_duplicate_name_under_same_parent(self):
        ...
        self.assertEqual(
            response.data,
            {"non_field_errors": ["Category name must be unique within its parent."]},
        )
    ...
    def test_rejects_unknown_parent(self):
        response = self.client.post(
            reverse("category-list"), {"name": "Water", "parent_id": 999}
        )
...
class CategoryUpdateTests(APITestCase):
    def test_updates(self):
        ...
        response = self.client.put(
            reverse("category-detail", args=[water.id]),
            {"name": "Still Water", "parent_id": None},
        )
    ...
    def test_rejects_missing_name(self):
        ...
        response = self.client.put(
            reverse("category-detail", args=[drinks.id]), {"parent_id": None}
        )
    ...
    def test_not_found(self):
        response = self.client.put(
            reverse("category-detail", args=[999]), {"name": "Drinks"}
        )
    ...
    def test_rejects_renaming_to_sibling_name(self):
        ...
        response = self.client.patch(
            reverse("category-detail", args=[juice.id]), {"name": "Water"}
        )
    ...
    def test_moves_under_another_parent(self):
        ...
        response = self.client.patch(
            reverse("category-detail", args=[water.id]), {"parent_id": food.id}
        )
    ...
    def test_rejects_self_as_parent(self):
        ...
        response = self.client.patch(
            reverse("category-detail", args=[drinks.id]), {"parent_id": drinks.id}
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data, {"parent_id": ["A category cannot be its own ancestor."]}
        )
    ...
    def test_patches_partially(self):
        ...
        response = self.client.patch(
            reverse("category-detail", args=[water.id]), {"name": "Still Water"}
        )
...
class CategoryDeleteTests(APITestCase):
    ...
    def test_rejects_category_with_children(self):
        ...
        self.assertEqual(
            response.data,
            {"detail": "Category still has products or child categories."},
        )
        self.assertEqual(Category.objects.count(), 2)
    ...
    def test_rejects_category_with_products(self):
        drinks = Category.objects.create(name="Drinks")
        Product.objects.create(
            title="Still Water 1.5L", sku="DRK-WAT-001", price_cents=99, category=drinks
        )
...
class ProductCreateTests(APITestCase):
    ...
    def test_rejects_missing_required_fields(self):
        response = self.client.post(
            reverse("product-list"), {"description": "Natural spring water"}
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            sorted(response.data), ["category_id", "price_cents", "sku", "title"]
        )
        self.assertFalse(Product.objects.exists())
...
class ProductUpdateTests(APITestCase):
    ...
    def test_rejects_missing_required_fields(self):
        ...
        response = self.client.put(
            reverse("product-detail", args=[product.id]), {"title": "Still Water"}
        )
    ...
    def test_not_found(self):
        response = self.client.put(
            reverse("product-detail", args=[999]), {"title": "Water"}
        )
    ...
    def test_patches_partially(self):
        ...
        response = self.client.patch(
            reverse("product-detail", args=[product.id]), {"price_cents": 129}
        )
